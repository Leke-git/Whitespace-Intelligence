...
export default function RegistryPage() {
  ...
  const fetchOrgs = useCallback(async () => {
    ...
    const [
      ...
      supabase
        .from('programme_lgas')
        .select('programme_id, lga_id, lga_gap_scores(name, state)')
    ]);
    ...
    if (orgError) {
      ...
      lgaData?.forEach(pl => {
        ...
        if (orgId) {
          ...
          if (lgaName && !lgaMap[orgId].includes(lgaName)) {
            lgaMap[orgId].push(lgaName);
          }
        }
      });
            ...
            >
              <div className="flex-grow overflow-y-auto p-5 space-y-5">
                <div className={`flex items-center justify-between mb-2 ${isMobile ? 'pr-12' : ''}`}>
                  <h2 className="text-xl font-bold text-slate-900 uppercase tracking-tight">Filters</h2>
                </div>
